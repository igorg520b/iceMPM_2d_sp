...
*/
    ...
    ZIRAN_FORCE_INLINE void
    column_rotation (MatrixType &a) const
    {
...
zero_chase (Matrix<T, 3, 3> &h, Matrix<T, 3, 3> &u, Matrix<T, 3, 3> &v)
{
    /**
          Reduce H to of form
          x x +
    ...
    /**
          ...
          0 x x
          0 + x
          Can calculate r2 without multiplying by r1 since both entries are in
     first two rows thus no need to divide by sqrt(a^2+b^2)
          */
...
template <class T>
inline ZIRAN_FORCE_INLINE void
make_upper_bidiag (Matrix<T, 3, 3> &h, Matrix<T, 3, 3> &u, Matrix<T, 3, 3> &v)
{
...
template <class T>
inline ZIRAN_FORCE_INLINE void
make_lambda_shape (Matrix<T, 3, 3> &h, Matrix<T, 3, 3> &u, Matrix<T, 3, 3> &v)
{
    u = Matrix<T, 3, 3>::Identity ();
    v = Matrix<T, 3, 3>::Identity ();
